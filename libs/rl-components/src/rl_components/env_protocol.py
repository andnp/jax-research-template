...
class EnvSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.action_low.shape != self.action_shape:
            raise ValueError(f"action_low shape must match action_shape {self.action_shape}, got {self.action_low.shape}")
        if self.action_high.shape != self.action_shape:
            raise ValueError(f"action_high shape must match action_shape {self.action_shape}, got {self.action_high.shape}")
        ...
        try:
            bounds_are_ordered = bool(jnp.all(self.action_low <= self.action_high))
