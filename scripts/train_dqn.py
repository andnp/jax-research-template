...
def main():
    config = DQNConfig(
        TOTAL_TIMESTEPS=100_000, NUM_ENVS=4, BUFFER_SIZE=50_000, BATCH_SIZE=128, LEARNING_STARTS=1000, TARGET_NETWORK_FREQUENCY=500, LR=1e-3, ENV_NAME="CartPole-v1", SEED=42
    )
    ...
    # Average over parallel environments
    mean_returns = returns.mean(axis=-1)

    print(f"Compilation Time: {max(0, total_time - execution_time):.2f}s")
    print(f"Execution Time:   {execution_time:.2f}s")
    print(f"SPS:              {sps:.2f}")
    print(f"Final Return:     {mean_returns[-1].item():.2f}")
    print(f"Max Return:       {mean_returns.max().item():.2f}")
