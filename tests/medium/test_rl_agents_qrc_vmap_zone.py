"""One compiled QRC kernel must train a batch of hyperparameter arms independently."""

import dataclasses

import jax
import jax.numpy as jnp
from rl_agents.qrc import QRCAgent, QRCAgentState, QRCConfig, QRCHypers, qrc_hypers
...
def _run_arm(agent: QRCAgent, base_hypers: QRCHypers, learning_rate: jax.Array, key: jax.Array):
    hypers = dataclasses.replace(base_hypers, LR=learning_rate)
    state = dataclasses.replace(agent.init(key, SPEC), hypers=hypers)

    def one_step(state: QRCAgentState, step_index: jax.Array):
        timestep = Timestep(
...
class TestVmapZone:
    def test_batched_learning_rates_train_independent_arms(self) -> None:
        ...
        """
        config = QRCConfig(LEARNING_STARTS=0, TRAIN_FREQUENCY=1, BATCH_SIZE=2, BUFFER_SIZE=64)
        agent = QRCAgent(config)
        base_hypers = qrc_hypers(config)
