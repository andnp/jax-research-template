"""One compiled kernel must train a batch of SAC hyperparameter arms independently."""

import dataclasses

import jax
import jax.numpy as jnp
from rl_agents.sac import SACAgent, SACAgentState, SACConfig, SACHypers, sac_hypers
...
def _run_arm(agent: SACAgent, base_hypers: SACHypers, learning_rate: jax.Array, key: jax.Array):
    hypers = dataclasses.replace(base_hypers, LR=learning_rate)
    state = dataclasses.replace(agent.init(key, SPEC), hypers=hypers)

    def one_step(state: SACAgentState, step_index: jax.Array):
        timestep = Timestep(
...
class TestVmapZone:
    def test_batched_learning_rates_train_independent_arms(self) -> None:
        ...
        """
        config = SACConfig(LEARNING_STARTS=0, TRAIN_FREQUENCY=1, BATCH_SIZE=2, BUFFER_SIZE=64)
        agent = SACAgent(config)
        base_hypers = sac_hypers(config)
