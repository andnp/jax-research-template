"""One compiled kernel must train a batch of TD3 hyperparameter arms independently."""

import dataclasses

import jax
import jax.numpy as jnp
from rl_agents.td3 import TD3Agent, TD3AgentState, TD3Config, TD3Hypers, td3_hypers
from rl_components.env_protocol import EnvSpec
from rl_components.timestep import Timestep

SPEC = EnvSpec(
    id="toy-continuous", observation_shape=(2,), action_shape=(1,), action_dtype=jnp.float32, num_actions=None
)


def _run_arm(agent: TD3Agent, base_hypers: TD3Hypers, learning_rate: jax.Array, key: jax.Array):
    hypers = dataclasses.replace(base_hypers, LR=learning_rate)
    state = dataclasses.replace(agent.init(key, SPEC), hypers=hypers)

    def one_step(state: TD3AgentState, step_index: jax.Array):
        timestep = Timestep(
...
class TestVmapZone:
    def test_batched_learning_rates_train_independent_arms(self) -> None:
        ...
        """
        config = TD3Config(LEARNING_STARTS=0, TRAIN_FREQUENCY=1, POLICY_DELAY=1, BATCH_SIZE=2, BUFFER_SIZE=64)
        agent = TD3Agent(config)
        base_hypers = td3_hypers(config)
