...
class TestActorCriticForward:
    ...
    def test_jit_compilation(self) -> None:
        ...
        @jax.jit
        def forward(params: Any, x: jax.Array) -> Any:
    ...
    def test_value_is_scalar_per_sample(self) -> None:
        net = ActorCritic(action_dim=2)
        key = jax.random.key(0)
        params = net.init(key, jnp.zeros((4,)))
...
class TestContinuousActorCriticForward:
    def test_output_shapes_and_action_bounds(self) -> None:
    ...
    def test_batch_output_shapes_reduce_per_sample(self) -> None:
