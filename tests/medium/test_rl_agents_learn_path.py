...
LOSS_METRIC_AGENTS = ("dqn", "sac", "td3", "greedy_ac")
"""Agents that publish their losses.

``double_dqn``, ``dueling_dqn`` and ``qrc`` compute a real loss and then discard it -- their
``_update_step`` binds ``loss`` and returns the raw environment ``info`` -- so three of the
