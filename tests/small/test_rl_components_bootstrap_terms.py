...
The expected ``(discount, episode_end, is_truncated)`` triples are written as literals, never
...
CASES: list[tuple[bool, bool, bool, TruncationPolicy, float, bool, bool]] = [
    # Continuation: no boundary, so the bootstrap survives under either policy.
    (False, False, False, "bootstrap", GAMMA, False, False),
    (False, False, False, "terminate", GAMMA, False, False),
    ...
    (False, False, True, "bootstrap", GAMMA, True, True),
    (False, False, True, "terminate", 0.0, True, True),
    ...
    (False, True, False, "bootstrap", GAMMA, True, True),
    (False, True, False, "terminate", 0.0, True, True),
    # Both truncation sources at once: still one truncation.
    (False, True, True, "bootstrap", GAMMA, True, True),
    (False, True, True, "terminate", 0.0, True, True),
    # Termination: no successor state to value, so the bootstrap dies under either policy.
    (True, False, False, "bootstrap", 0.0, True, False),
    (True, False, False, "terminate", 0.0, True, False),
    # TERMINATION DOMINANCE. A step flagged terminated alongside either truncation source
    # is a termination: discount 0.0 under BOTH policies, and `is_truncated` False so the
    # step is never reported as a truncation. Treating it as a truncation would bootstrap
    # from a state nothing follows and bias every value estimate upward.
    (True, False, True, "bootstrap", 0.0, True, False),
    (True, False, True, "terminate", 0.0, True, False),
    (True, True, False, "bootstrap", 0.0, True, False),
    (True, True, False, "terminate", 0.0, True, False),
    (True, True, True, "bootstrap", 0.0, True, False),
    (True, True, True, "terminate", 0.0, True, False),
]
...
def _case_id(case: tuple[bool, bool, bool, TruncationPolicy, float, bool, bool]) -> str:
    terminated, truncated, cutoff_reached, policy, _, _, _ = case
...
class TestBootstrapTermsTable:
    ...
    def test_matches_expected_table(self, case: tuple[bool, bool, bool, TruncationPolicy, float, bool, bool]) -> None:
        (
            terminated,
            truncated,
            cutoff_reached,
        ...
        assert bool(is_truncated) is expected_is_truncated
...
class TestTerminationDominance:
    ...
    def test_termination_kills_bootstrap_despite_truncation(self, policy: TruncationPolicy, truncation_source: str) -> None:
        ...
        assert bool(is_truncated) is False


class TestBootstrapTermsDtypes:
    def test_discount_is_float32_and_flags_are_bool(self) -> None:
        discount, episode_end, is_truncated = bootstrap_terms(
...
class TestBootstrapTermsUnderJit:
    @pytest.mark.parametrize("case", CASES, ids=[_case_id(case) for case in CASES])
    def test_jit_with_policy_closed_over(self, case: tuple[bool, bool, bool, TruncationPolicy, float, bool, bool]) -> None:
        (
            terminated,
            truncated,
            cutoff_reached,
        ...
        discount, episode_end, is_truncated = jitted(
            jnp.asarray(terminated, jnp.bool_),
            jnp.asarray(truncated, jnp.bool_),
            jnp.asarray(cutoff_reached, jnp.bool_),
        )
